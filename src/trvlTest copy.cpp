...
int compLZ77(const std::deque<T> &inputDeque, std::deque<Entry<T>> &outBuf)
{
    ...
    while (lookL != lookR)
    {
        ...
        if (searchBuffer.size() > searchWindow)
        {
            searchBuffer.erase(searchBuffer.begin(), searchBuffer.begin() + (searchBuffer.size() - searchWindow));
        }
        ...
        entry.reset();
        entry = getLargestSubvec<T>(searchL, searchR, lookL, lookR, inputDeque.end());
        if (zeros > 0)
        {
            entry.z_ = true;
            entry.z = zeros;
        ...
        if (distance(lookL, inputDeque.end()) - l > 0)
        {
            lookL += l;
        }
        else
        {
            lookL = inputDeque.end();
        }
        if (distance(lookR, inputDeque.end()) - l > 0)
        {
            lookR += l;
        }
        else
        {
            lookR = inputDeque.end();
        }
    }
    return 1;
}
...
int toSerial(std::vector<bool> &serial, char input)
{
    ...
    nibble.insert(nibble.begin(), input & (1 << 0));
    nibble.insert(nibble.begin(), input & (1 << 1));
    nibble.insert(nibble.begin(), input & (1 << 2));
    nibble.insert(nibble.begin(), input & (1 << 3));
...
int toSerial(std::vector<bool> &serial, int &input, int bitsToWrite)
{
    ...
    while (bitsToWrite > 0)
    {
        ...
        {
            serial.insert(serial.end(), 1);
        }
        for (int i = 2; i >= 0; --i)
        {
            serial.insert(serial.end(), ((input & 0x07) & (1 << i)));
        }
        bitsToWrite -= 3;
        input >>= 3;
    }
...
template <typename T>
int compressLZRVL(std::deque<T> &pixel_diffs, std::vector<bool> &serialized)
{
    ...
    while (compressed.begin() != compressed.end())
    {
        ...
        toSerial(serialized, control);
        if (entry.z_)
        {
            assert(entry.z > 0);
            int z = toZigZag(entry.z);
            toSerial(serialized, z, ceil(log2(z + 1))); // run of zeros
        }
        if (entry.sv_)
        {
            assert(entry.d > 0);
            ...
            toSerial(serialized, d, ceil(log2(d + 1))); // distance
            toSerial(serialized, l, ceil(log2(l + 1))); // length
        }
        if (entry.c_)
        {
            int c = toZigZag(entry.c);
            // the character (delta) can be between -2^16/2 to 2^16/2
            toSerial(serialized, c, ceil(log2(c + 1))); // char
        }
...
Result run_trvl(InputFile &input_file, short change_threshold, int invalidation_threshold)
{
    ...
    Timer compression_timer;
    compressLZRVL(allPixelDiffs, serialized);
    float compression_time_sum = compression_timer.milliseconds();

    Timer decompression_timer;
    int success = decompressLZRVL(finalDecompression, serialized);
    float decompression_time_sum = decompression_timer.milliseconds();
    std::cout << "decompression finished" << std::endl;

    std::cout << std::endl;
    if (check(allPixelDiffs, finalDecompression))
    {
    ...
    std::cout << "total uncompressed size (diff) = " << frame_size * frame_count * sizeof(short) * 8 << std::endl;
    ...
    std::cout << "total compressed size (diff + (RVL)keyframe) = " << serialized.size() + rvl_frame.size() * sizeof(char) * 8 << std::endl;
    std::cout << "overall average compression ratio (diff + (RVL)keyframe))= in / out = " << float(allPixelDiffs.size() * sizeof(short) * 8 + frame_size * sizeof(short) * 8) / float(serialized.size() + rvl_frame.size() * sizeof(char) * 8) << std::endl;
    return Result(average_compression_time, average_decompression_time, 0, 0);
}
