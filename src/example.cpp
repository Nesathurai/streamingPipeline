...
void run_video()
{
    ...
    const std::string DATA_FOLDER_PATH = "/home/sc/streamingPipeline/analysisData/temporal-rvl-data/";
...
int main()
{
    ...
    // cv::Mat img = cv::imread("/home/sc/streamingPipeline/analysisData/temporal-rvl-data/frame_0_camera_0_depth.png");
    // // cv::imwrite

    // short CHANGE_THRESHOLD = 10;
    // int INVALIDATION_THRESHOLD = 2;

    // InputFile input_file(create_input_file("/home/sc/streamingPipeline/analysisData/temporal-rvl-data/", ""));
    
    // trvl::Encoder encoder(1920*1080, CHANGE_THRESHOLD, INVALIDATION_THRESHOLD);
    // trvl::Decoder decoder(1920*1080);

    // std::vector<short> depth_buffer(1920*1080);

    // auto trvl_frame = encoder.encode(depth_buffer.data(), 0);
    // auto depth_image = decoder.decode(trvl_frame.data(), 0);
