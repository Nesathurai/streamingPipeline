...
void vertices_to_uv(Eigen::MatrixXf camera_intrinsic, Eigen::MatrixXf *triangle_vertices, Eigen::MatrixXd *uv) {
    Eigen::Vector3f zero_vector = {0, 0, 0};
    // projection matrix
    camera_intrinsic.conservativeResize(camera_intrinsic.rows(), camera_intrinsic.cols() + 1);
    camera_intrinsic.col(camera_intrinsic.cols() - 1) = zero_vector;

    Eigen::MatrixXf triangle_vertices_homo = triangle_vertices->rowwise().homogeneous();
...
void multi_vertices_uv_to_triangle_uv(std::vector<Eigen::MatrixXd> *vertices_uv, Eigen::MatrixXi *triangle_indices_m, 
                                        Eigen::VectorXi *min_indeces, std::vector<Eigen::Vector2d> *mesh_uvs) {
    ...
    for (int i = 0; i < triangle_indices_m->rows(); i++) {
        ...
        Eigen::Vector2d texture_uv_0 = vertices_uv->at(min_index_0).row(vertex_index(0)) + shift_val * min_index_0;
        Eigen::Vector2d texture_uv_1 = vertices_uv->at(min_index_0).row(vertex_index(1)) + shift_val * min_index_0;
        Eigen::Vector2d texture_uv_2 = vertices_uv->at(min_index_0).row(vertex_index(2)) + shift_val * min_index_0;
...
void tensor_vertices_uv_to_triangle_uv(std::vector<Eigen::MatrixXd> *vertices_uv, Eigen::MatrixXi *triangle_indices_m, 
                                        Eigen::VectorXi *min_indeces, core::Tensor *mesh_uvs) {
    ...
    for (int i = 0; i < triangle_indices_m->rows(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            Eigen::Vector2f texture_uv = float_vertices_uv.at(min_index).row(vertex_index(j)) + shift_val * min_index;
...
void optimized_multi_cam_uv(t::geometry::TriangleMesh *mesh,
                            std::vector<core::Tensor> intrinsic_list, std::vector<core::Tensor> extrinsic_tf_list,
                            std::vector<cv::Mat> *cv_depth_img_list) {

    std::vector<Eigen::MatrixXf> camera_intrinsic_list;
    for (uint32_t i = 0; i < device_count; i++) {
        Eigen::MatrixXf camera_intrinsic = core::eigen_converter::TensorToEigenMatrixXf(intrinsic_list.at(i));
    ...
    Eigen::MatrixXi triangle_indices_m = core::eigen_converter::TensorToEigenMatrixXi(triangle_indices);
    Eigen::MatrixXf triangle_vertices_m = core::eigen_converter::TensorToEigenMatrixXf(triangle_vertices);
    ...
    for (uint32_t i = 0; i < device_count; i++) {
        ...
        Eigen::MatrixXf eigen_intrinsic = core::eigen_converter::TensorToEigenMatrixXf(intrinsic_list.at(i));
        Eigen::MatrixXf eigen_extrinsics = core::eigen_converter::TensorToEigenMatrixXf(extrinsic_tf_list.at(i));
        transform_vertices(&triangle_vertices_m, eigen_extrinsics, &triangle_vertices_tf);
        ...
        multi_occlusion_test(&(cv_depth_img_list->at(i)), &triangle_vertices_tf, camera_intrinsic_list.at(i), &z_error);
    ...
    core::Tensor tensor_mesh_uvs({num_triangles, 3, 2}, core::Float32, core::Device("CPU:0"));
    tensor_vertices_uv_to_triangle_uv(&mesh_uvs, &triangle_indices_m, 
                                        &min_indeces, &tensor_mesh_uvs);


    mesh->SetTriangleAttr("texture_uvs", tensor_mesh_uvs);
}

void multi_occlusion_test(cv::Mat *cv_depth_img, Eigen::MatrixXf *float_vertices_tf, 
                    Eigen::MatrixXf camera_intrinsic, Eigen::VectorXf *z_error) {
    Eigen::MatrixXd uvs;
    ...
    for (int i = 0; i < uvs.rows(); i++) {
        ...
        if (uv(0) < 0 || uv(0) > H_RES || uv(1) < 0 || uv(1) > V_RES) {
            ...
            float depth_val = static_cast<float>(cv_depth_img->at<uint16_t>(uv(1), uv(0)));
            depth_m(i) = depth_val;
        }
    }
...
void vertices_uv_to_triangle_uv(Eigen::MatrixXd *vertices_uv, Eigen::MatrixXi *triangle_indices_m, 
...
void transform_vertices(Eigen::MatrixXf *triangle_vertices_m, Eigen::MatrixXf extrinsic_tf, Eigen::MatrixXf *transformed_vertices) {
    Eigen::MatrixXf triangle_vertices_m_homo = triangle_vertices_m->rowwise().homogeneous();
    *transformed_vertices = (extrinsic_tf * triangle_vertices_m_homo.transpose()).transpose().rowwise().hnormalized();
}

void mesh_uv_mapping(t::geometry::TriangleMesh *mesh, core::Tensor intrinsic_t, 
    ...
    Eigen::MatrixXf eigen_extrinsic_tf = core::eigen_converter::TensorToEigenMatrixXf(extrinsic_tf);
    Eigen::MatrixXf transformed_vertices;
    transform_vertices(&triangle_vertices_m, eigen_extrinsic_tf, &transformed_vertices);

    Eigen::MatrixXf eigen_intrinsic = core::eigen_converter::TensorToEigenMatrixXf(intrinsic_t);
    Eigen::MatrixXd uv_m;
    ...
    int num_triangles = triangle_indices_m.rows();
    core::Tensor mesh_uvs({num_triangles, 3, 2}, core::Float32, core::Device("CPU:0"));
...
float occlusion_test(t::geometry::Image depth_img, Eigen::MatrixXf float_vertices_tf, 
    ...
    if ((max_uv(0) < 0 || max_uv(0) > 1920) || (max_uv(1) < 0 || max_uv(1) > 1080)) {
        return 100000;
    } else {
...
int calc_camera_triangle_angle(Eigen::Vector3d normal, Eigen::MatrixXf float_vertices, 
    ...
    for (int i = 0; i < camera_pos.size(); i++) {
        ...
        Eigen::MatrixXf eigen_extrinsic = core::eigen_converter::TensorToEigenMatrixXf(extrinsic_tf_list->at(i));
        transform_vertices(&float_vertices, eigen_extrinsic, &float_vertices_tf);
        occlusion_result.push_back(occlusion_test(depth_img_list->at(i), float_vertices_tf, camera_intrinsic_list->at(i)));
    }

    int best_camera = std::distance(std::begin(occlusion_result), 
                                    std::min_element(std::begin(occlusion_result), std::end(occlusion_result)));
    return best_camera;
}

void multi_cam_uv(geometry::TriangleMesh *cpu_mesh, std::vector<Eigen::Vector3d> camera_pos, 
                    std::vector<core::Tensor> intrinsic_list, std::vector<core::Tensor> extrinsic_tf_list,
                    std::vector<t::geometry::Image> depth_img_list,
    ...
    for (int i = 0; i < depth_img_list.size(); i++) {
        depth_img_cpu_list.push_back(depth_img_list.at(i).To(core::Device("CPU:0")));
    }
    ...
    for (int i = 0; i < triangles.size(); i++) {
        ...
        // select best camera
        int best_camera = calc_camera_triangle_angle(normal, float_vertices, camera_pos, 
                                                        &depth_img_cpu_list, &camera_intrinsic_list, &extrinsic_tf_list);

        // find u and v in that camera view
        Eigen::MatrixXf eigen_extrinsic = core::eigen_converter::TensorToEigenMatrixXf(extrinsic_tf_list.at(best_camera));
        Eigen::MatrixXf float_vertices_tf;
        transform_vertices(&float_vertices, eigen_extrinsic, &float_vertices_tf);


        Eigen::MatrixXd uv;
        vertices_to_uv(camera_intrinsic_list.at(best_camera), &float_vertices_tf, &uv);
    ...
    core::Tensor uv_tensor = core::eigen_converter::EigenVector2dVectorToTensor(triangle_uv, core::Float32, core::Device("CPU:0"));

    Eigen::MatrixXd uv_m = core::eigen_converter::TensorToEigenMatrixXd(uv_tensor);
